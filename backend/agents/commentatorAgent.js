// commentatorAgent.js — Live in-round commentary (OpenAI gpt-4o-mini).
...
const TEMPLATES = {
  english: {
    word_found: ['Nice find! Keep the streak going.', 'Sharp eyes — {wordsFound}/{totalWords} now.', 'Another one down — momentum building!'],
    streak: ['{streak} in a row — you are on fire!', 'Streak {streak}! Multiplier active.', 'Combo {streak} — keep cooking!'],
    half_time: ['Half time! {wordsFound}/{totalWords} found. Push harder.', 'Halfway done — focus the search.'],
    low_time: ['Only {timeLeft}s left — grab anything!', 'Final stretch — hurry!'],
    idle: ['Look across rows AND columns. Diagonals too!', 'Try starting from a rare letter — like Q or Z.', 'Stuck? A hint costs only 30 points.'],
  },
  urdu: {
    word_found: ['Sahi! Aur {totalWords} mein se {wordsFound} ho gaye.', 'Acha kaam — momentum bana ke rakho.', 'Ek aur mil gaya, keep going!'],
    streak: ['{streak} in a row — bilkul fire pe ho!', 'Streak {streak}! Combo multiplier active.', 'Combo {streak} — kamaal!'],
    half_time: ['Half time! {wordsFound}/{totalWords} ho gaye. Speed up.', 'Aadha time gaya — focus karo.'],
    low_time: ['Sirf {timeLeft} second baqi — jaldi karo!', 'Final stretch — koi bhi word grab karo!'],
    idle: ['Letters dhyaan se dekho — diagonals bhi try karo.', 'Q ya Z jaisa letter dhoondh ke wahan se shuru karo.', 'Stuck ho? Hint sirf 30 points ka hai.'],
  },
};
